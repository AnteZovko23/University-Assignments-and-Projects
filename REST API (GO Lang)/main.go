...
func main() {
	...
	users.GET("/searchResult", ensureLoggedIn(), searchResult)

	users.GET("/test", showTest)

	users.GET("/register", ensureNotLoggedIn(), showRegistrationPage)

	users.POST("/homepage", ensureLoggedIn(), showHomePage)

	users.GET("/homepage", ensureLoggedIn(), showHomePage)
	...
	router.GET("/all", ensureLoggedIn(), func(c *gin.Context) {
		...
		offset2 := limit * page
		offsetsandbox9644c270346740bbb12b7a810d003cb3.mailgun.org := strconv.Itoa(offset2)

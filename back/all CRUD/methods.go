...
func DeleteItemsByField(str string, tables []string, c *gin.Context) string {
...
func UpdateItemsByField(str string, tables []string, c *gin.Context) string {
	...
	for i := 0; i < len(tables); i++ {
		...
		_, err = stmt.Exec(name, description, category, barcode, query)
		if err != nil {
			fmt.Println(err.Error())
		}
	}

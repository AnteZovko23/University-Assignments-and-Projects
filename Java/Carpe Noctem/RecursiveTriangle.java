...
/**
 * 
 * 3 Freshman Semester Comp Sci Labs with a graphical user interface
 * 
 * @author Ante Zovko
...
public class RecursiveTriangle {
	...
	private JDialog lightPicker, game, playerCount;

	private ButtonGroup buttonGroup;
	private JRadioButton noLightShow, dotLightShow, backgroundLightshow, fullLightShow;

	private JFrame dotFrame;
	private GCanvas dotCourse;

	private JPanel jp, dotJP;

	private JLabel l, l2, lbl, lbl2, lbl3, lbl4, lbl5, lbl6, lbl7, lbl2nx, lbl3nx, lbl4nx, lbl5nx, lbl6nx, lbl7nx,
			dotLbl, dotLbl2, dotWarning, dotWarning2, gameHeader, scoreHead, firstPlayerScore, secondPlayerScore,
			thirdPlayerScore, fourthPlayerScore, fifthPlayerScore;

	private JTextField textField, textField2, textField3, textField4, textField5, textField6, textField7, dotTextField,
			dotTextField2, jjj, gameField, gameField2, gameField3, gameField4, gameField5, mess, mess2;

	private JButton execute, reset, defaultVer, randomColors, pickColors, lightShow, dotStartRace, dotBackgroundColors,
			dotBack, dotDefault, dotReset, randomDots, gameStartRace, gameResetRace, gameResetScore, againstOne,
			againstTwo, againstThree, againstFour, b, can;

	private JCheckBox box, jcb;

	private JSeparator gameSecondSeparator, gameFirstSeparator;

	// Menu Components
	private JMenuBar jmb, dotJMB;

	private JMenu file, newWindow, centroid, view, centroidToggle, sierpinskiToggle, sierpinski, help, dotFile, newGame,
			dotView, stageColor, dotRaceOptions;

	private JMenuItem saveImg, closeWindow, quit, randomCentroidToggle, specificCentroidToggle, randomSierpinskiToggle,
			specificSierpinskiToggle, randomSierpinskiWindow, specificSierpinskiWindow, dotRace, defaultMenu, resetMenu,
			unlimitedAgainstAI, about, newRandomWindow, newPickWindow, unlimitedAgainstPlayer, dotClose, dotQuit,
			menuLightShow, stageRandom, stageSpecific, menuRandomDots, menuDefaultDots, menuClearDots, menuStartRace,
			menuResetRace;

	// Data
	private Timer timer;

	private ArrayList<GOval> dotArr;
	private ArrayList<Color> dotColors;
	private ArrayList<JTextField> casinoArr;
	private ArrayList<JLabel> scoreKeeper;

	private Color line1, line2, line3, dotColor, dotRestColor, c, casinoRed;

	private boolean isPlayerCountClosed, isAgainstPlayers;
	private static int numOfWindows = 0;
	private int x1, x2, x3, y1, y2, y3, depth, amountOfDots, farthestX, speed, space, dotSize, amountOfPlayers;
	...
	/**
	 * Constructor that summons the First Window and gives centroid listeners
	 * 
	 */
	...
	private void createWindows() {
		...
		mainFrame = new JFrame("Carpe Noctem v1.0.0 29/4/2019");
	 ...
	 * 
	 */
	private void centroid(int x1, int y1, int x2, int y2, int x3, int y3, int depth) {
	...
	/**
	 * 
	 * Recursive method that creates a sierpinski triangle
	 * 
	 * @param x1    coordinate
	 * @param y1    coordinate
	 * @param x2    coordinate
	 ...
	 * 
	 **/
	private void sierpinski(int x1, int y1, int x2, int y2, int x3, int y3, int depth) {
	...
	/**
	 * Resets Everything about the centroid
	 * 
	 */
	...
	/**
	 * 
	 * Resets everything about the sierpinski
	 * 
	 */
	...
	/**
	 * Generates random number between 0 and 255 inclusive
	 *
	...
	/**
	 * Checks user input for invalid Characters
	 * 
	...
	/**
	 * Allows Centroid dynamic coordinate Change
	 * 
	 */
	...
	/**
	 * Allows Sierpinski dynamic coordinate Change
	 * 
	 */
	...
	/**
	 * Checks for sierpinski bad input
	 * 
	 */
	...
	private void createDotRace() {
		...
		lightPicker = new JDialog(dotFrame, "Lightshow Chooser", Dialog.ModalityType.TOOLKIT_MODAL);
		lightPicker.setResizable(false);
		lightPicker.setSize(300, 300);
		lightPicker.setVisible(false);
		lightPicker.setAlwaysOnTop(true);
		lightPicker.setLayout(null);
		...
		lightPicker.add(backgroundLightshow);
		...
		lightShow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				lightPicker.setLocationRelativeTo(dotFrame);
				lightPicker.setVisible(true);
	...
	/**
	 * Creates a background lightshow
	 * 
	 */
	...
	/**
	 * Draws existing dots on a canvas
	 * 
	 */
	...
	/**
	 * Determines the leading X Coordinate
	 * 
	 * @return the leading X Coordinate
	 */
	...
	/**
	 * Gets the leading dot number
	 * 
	 * @return the leading dot number
	 */
	...
	/**
	 * Dot text field input validation
	 * 
	 * @see Lab 7, St. Bonaventure University: "Password Utils.java"
	 * @return
	 */
	...
	/**
	 * Game text field input validation
	 * 
	 * @return true iff it is invalid
	 */
	...
	/**
	 * Creates a new dot blank canvas
	 * 
	 */
	...
	/**
	 * Resets everything about the dot
	 * 
	 */
	...
	/**
	 * Dot text fields input handling
	 * 
	 * 
	 */
							...
							if (ke.getKeyCode() == 40 && Integer.parseInt(dotTextField.getText()) == 2) {
								...
								if (!(ke.getKeyCode() == 10 || ke.getKeyCode() == 37 || ke.getKeyCode() == 39)) {
									...
									for (int i = 0; i < amountOfPlayers; i++) {
										...
										if (!isAgainstPlayers) {
											casinoArr.get(i).setEditable(false);
										} else {
